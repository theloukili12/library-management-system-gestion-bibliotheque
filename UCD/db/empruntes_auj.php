<?php 
    ...
    class myPDF extends FPDF{
        function header(){
            ...
            $this-> Ln(30);
            $this-> SetLineWidth(500.0);
            $sql = "select Count(*) as nbr from pret p 
            ...
            $this->Cell(250,0,'Les empruntes d\'Aujourd\'hui (Order by Date d\'emprunt ) est : '.$nbrp.' Livres',0,0,'C');
            $this->Ln(10);
        ...
        function ViewTable($db){
            ...
            $stmt = $db->query("Select l.titreliv ,p.cinadm , etd.cne_etd ,p.dateretour, p.dateaccord from pret p , exemplaire e , livre l ,etudiant etd where p.codexp = e.codexp && l.codliv = e.codliv AND DAY(p.dateaccord) = DAY(NOW())AND MONTH(p.dateaccord) = MONTH(NOW())AND YEAR(p.dateaccord) = YEAR(NOW())
            AND p.cne_etd = etd.cne_etd");
            while($data=$stmt->fetch(PDO::FETCH_OBJ)){
                $this->SetFillColor(224,235,255);
                $this->SetTextColor(0);
                $this->SetFont('');
                $this->Cell(55,10,$data->titreliv,'LR',0,'L');
